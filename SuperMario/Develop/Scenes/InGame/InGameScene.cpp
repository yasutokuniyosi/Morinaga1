...
void InGameScene::Initialize()
{
	...
	// モードによって、生成するオブジェクトを変える
			// 生成位置の情報
	player = CreateObject<Player>(Vector2D(100,295));
	enemybase = CreateObject<EnemyBase>(Vector2D(150, 295));
...
void InGameScene::LoadStageMapCSV() const
{
	...
	FILE* fp = NULL;
	//指定ファイルを読み込む
	fopen_s(&fp, "C:/PG/SuperMario/Develop/Map/SuperMario_Map.csv", "r");
